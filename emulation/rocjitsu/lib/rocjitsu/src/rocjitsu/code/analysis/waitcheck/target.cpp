...
[[nodiscard]] uint32_t WaitcheckTarget::maximum_dependency_wait(rj_code_arch_t arch,
...
WaitcheckTarget::counter_no_wait_value(rj_code_arch_t arch, WaitCounterKind counter) {
  if (uses_legacy_waitcnt(arch)) {
    ...
    case WaitCounterKind::Exp:
      return 0x07u;
    case WaitCounterKind::VmVsrc:
      return has_legacy_vscnt(arch) ? std::optional<uint32_t>{0x07u} : std::nullopt;
    case WaitCounterKind::VaVdst:
      return has_legacy_vscnt(arch) ? std::optional<uint32_t>{0x0fu} : std::nullopt;
    default:
      return std::nullopt;
    }
  }
...
[[nodiscard]] std::optional<std::array<std::optional<uint32_t>, kCounterCount>>
WaitcheckTarget::explicit_wait_fields(const Instruction &inst, rj_code_arch_t arch) {
  ...
  if ((has_legacy_vscnt(arch) && mnemonic == "s_waitcnt_depctr") ||
      (!uses_legacy_waitcnt(arch) && mnemonic == "s_wait_alu")) {
    const auto value = operand_value(0);
    if (!value)
      return std::nullopt;
...
[[nodiscard]] bool WaitcheckTarget::is_cdna4_mubuf_lds_load(const Instruction &inst,
  ...
  if ((arch != ROCJITSU_CODE_ARCH_CDNA3 && arch != ROCJITSU_CODE_ARCH_CDNA4) ||
      !starts_with(inst.mnemonic(), "buffer_load") || inst.raw_encoding() == nullptr ||
...
[[nodiscard]] bool WaitcheckTarget::is_scalar_memory_op(std::string_view mnemonic) {
  return starts_with(mnemonic, "s_load") || starts_with(mnemonic, "s_buffer_load") ||
         ...
         starts_with(mnemonic, "s_atomic_") || starts_with(mnemonic, "s_buffer_atomic_") ||
         starts_with(mnemonic, "s_scratch_load") || starts_with(mnemonic, "s_scratch_store") ||
         starts_with(mnemonic, "s_buffer_prefetch_") || starts_with(mnemonic, "s_dcache_") ||
...
[[nodiscard]] bool WaitcheckTarget::is_gfx1250_smem(std::string_view mnemonic) {
...
[[nodiscard]] std::vector<ClassifiedEvent> WaitcheckTarget::classify_events(const Instruction &inst,
                                                                            rj_code_arch_t arch) {
  ...
  if (is_image_atomic(mnemonic)) {
    ...
    if (inst.raw_encoding() != nullptr && inst.size() >= 8) {
      returns_value = uses_legacy_waitcnt(arch) ? (inst.raw_encoding()[0] & (1u << 14u)) != 0
  ...
  if (is_vmem_atomic(mnemonic)) {
    ...
    const WaitCounterKind counter =
        returns_value ? WaitCounterKind::Load : vmem_store_wait_counter(arch);
    const WaitEventKind kind =
        starts_with(mnemonic, "flat_atomic")
  ...
  if (uses_ds_wait_counter(mnemonic)) {
    ...
    const bool gds = uses_legacy_waitcnt(arch) &&
                     (mnemonic == "ds_ordered_count" || mnemonic == "ds_add_gs_reg_rtn" ||
                      mnemonic == "ds_sub_gs_reg_rtn" || starts_with(mnemonic, "ds_gws_") ||
  ...
  if (mnemonic == "s_sendmsg" || mnemonic == "s_sendmsghalt" || mnemonic == "s_sendmsg_rtn_b32" ||
    ...
    const bool returns_value = has_register_result();
    events.emplace_back(smem_wait_counter(arch), WaitEventKind::SqMessage,
                        returns_value ? TrackedRegisterSource::Defs : TrackedRegisterSource::None,
