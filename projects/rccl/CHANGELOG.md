...
* Added optional `ENABLE_NET_PROFILING` to emit `NET_IB` / `NET_SOCK` profiler events from the built-in transports.
...
* Fixed inspector teardown producing no dump when `NCCL_INSPECTOR_DUMP_THREAD_ENABLE=0`, and Prometheus teardown output being unlinked immediately when no periodic dump thread ran.
* Fixed the example profiler `% 0` crash when a pool-size environment variable is set to 0.
