...
| `rocprof_trace_decoder.hidden_latency` | Instruction-pipe overlap and hidden-latency analysis | Public utility |
...
## `rocprof_trace_decoder.hidden_latency`

`hidden_latency.py` estimates how many instruction cycles overlap concurrent
instruction-pipe activity. `analyze_hidden_latency()` takes decoded records keyed
by shader engine and an optional `CodeIndex`:
...
`max(CodeEntry.latency + CodeEntry.idle - HiddenLatency.total(), 0)` because
interval coalescing can make hidden latency exceed the aggregate total.

The estimator builds busy intervals per `(shader engine, SIMD)` using the
priority `MATRIX > VALU > VMEM/LDS/FLAT > SMEM/SALU`. Pass a `CodeIndex` so
instructions beginning with `v_mfma*`, `v_smfma*`, `v_wmma*`, or `v_swmma*`
can be classified as MATRIX.

On gfx9, `HiddenLatency.issue` represents overlap with issue time. On gfx10+,
it represents overlap with execution time. MI200 and MI300 results are
therefore a coarser estimate than gfx10+ results.
